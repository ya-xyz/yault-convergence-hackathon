...
Before submitting any attestation, the workflow queries **3 independent external data sources** and gates the attestation on their results:
...
All three checks run **in parallel** via `Promise.all`. If vault balance is zero or compliance screening fails, the attestation is **aborted** before any on-chain write.

The final `evidenceHash` written on-chain is:

```
keccak256(original_evidence ‖ drand_round ‖ drand_randomness ‖ vault_totalAssets ‖ compliance_checkId)
```

This makes every attestation verifiably linked to external reality (time, asset state, compliance).
...
- **Cron trigger**: Runs on a schedule (e.g. every 5 minutes). Optionally calls `GET {platformApiBaseUrl}/api/oracle/pending` to fetch pending attestation requests; if any, runs the 3 external data source checks, then builds `submitAttestation(SOURCE_ORACLE, ...)` and submits via CRE EVM Write to `ReleaseAttestation`.
...
- `oracle/workflow/src/main.ts` — CRE workflow (triggers, 3 external data sources, EVM write)
