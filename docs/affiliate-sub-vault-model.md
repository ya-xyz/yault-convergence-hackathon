...
### Phase 3A: Sub-Vault MVP (Month 3–4)

| Task | Description | Effort |
|------|-------------|--------|
| `SubVault.sol` | Core agent vault: collateral, borrow, repay, health factor | 5 days |
| `SubVaultFactory.sol` | Factory for deploying per-agent Sub-Vaults | 2 days |
| `CollateralManager.sol` | Collateral tracking, health monitoring, liquidation | 5 days |
| `AffiliateRegistry.sol` | Agent registration, tier management, suspension | 3 days |
| `RepaymentRouter.sol` | Interest routing from Sub-Vaults to main pool | 3 days |
| `AffiliateStrategy.sol` | IYaultStrategy adapter for StrategyAllocator | 3 days |
| Integration tests | Full lifecycle: register → deposit → borrow → lend → repay → harvest | 5 days |
| Chainlink Automation setup | Health monitoring + interest accrual jobs | 3 days |
| Security audit | Third-party review of all affiliate contracts | 2–3 weeks |

### Phase 3B: Agent Operations (Month 4–5)

| Task | Description | Effort |
|------|-------------|--------|
| Agent onboarding portal | Web UI for KYC, business plan submission, collateral deposit | 1 week |
| Agent dashboard | Sub-Vault status, loan tracking, health factor, P&L | 1 week |
| Chainlink Functions: credit scoring | Off-chain credit score computation per agent | 3 days |
| Chainlink Functions: borrower verification | Off-chain business entity verification | 3 days |
| Seed agent recruitment | Identify and onboard 2–3 pilot agents | 2 weeks |
| Pilot testing | Live operation with small amounts ($10K–50K per agent) | 4 weeks |

### Phase 3C: Scale (Month 5+)

| Task | Description |
|------|-------------|
| Agent rate bidding | Agents compete for pool capital by offering higher interest rates |
| On-chain reputation | Transparent credit scores based on repayment history, loss rates |
| Cross-chain Sub-Vaults | Agents on L2s borrow from mainnet pool via Chainlink CCIP |
| Dynamic leverage | Chainlink Functions adjusts leverage limits based on real-time risk |
| Governance integration | Token holders vote on agent approvals, tier upgrades, parameter changes |
| Insurance pool expansion | Dedicated insurance pool with yield-bearing reserves |
