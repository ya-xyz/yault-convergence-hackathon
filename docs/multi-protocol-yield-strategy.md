...
### Phase 1: Strategy Framework (Month 1–2)

**Goal**: Decouple Aave from YaultVault, establish multi-strategy infrastructure.

| Task | Description | Dependencies | Effort |
|------|-------------|-------------|--------|
| Define `IYaultStrategy` interface | Common interface for all strategies | None | 1 day |
| Extract `AaveV3Strategy.sol` | Move Aave logic from vault to standalone adapter | Interface | 2 days |
| Build `StrategyAllocator.sol` | Weight-based routing, rebalance, emergency withdraw | Interface | 5 days |
| Modify `YaultVault.sol` | Replace direct Aave calls with allocator routing | Allocator | 3 days |
| Build `OndoOUSGStrategy.sol` | OUSG mint/redeem integration | Interface | 3 days |
| Build `MorphoStrategy.sol` | Morpho Blue supply/withdraw integration | Interface | 3 days |
| Update `AutoHarvest.sol` | Multi-strategy harvest loop | Allocator | 2 days |
| Comprehensive test suite | Unit tests + integration tests + fuzz tests | All above | 5 days |
| Security audit | Third-party review of new contracts | All above | 2 weeks |

**Deliverable**: YaultVault operates with 3 strategies (Aave, OUSG, Morpho) via the allocator. Existing functionality preserved.

### Phase 2: Term Premium + Analytics (Month 2–3)

**Goal**: Add Pendle PT strategy and upgrade analytics for multi-strategy reporting.

| Task | Description | Dependencies | Effort |
|------|-------------|-------------|--------|
| Build `PendlePTStrategy.sol` | PT purchase, maturity redemption, AMM exit | Phase 1 | 5 days |
| Implement PT rollover logic | Auto-roll into new PTs at maturity | PendlePTStrategy | 3 days |
| Chainlink Automation: rebalance job | Periodic drift detection and rebalancing | Allocator | 2 days |
| Chainlink Automation: PT maturity job | Trigger rollover near maturity | PendlePTStrategy | 2 days |
| Update `PortfolioAnalytics.sol` | Multi-strategy APY, risk score, Sharpe ratio | Phase 1 | 3 days |
| Update `ChainlinkPriceFeedTracker` | Multi-strategy NAV with new price feeds | Phase 1 | 2 days |
| Integration tests | End-to-end test with all 4 strategies | All above | 3 days |
| Security audit | Review Pendle integration + new Automation jobs | All above | 2 weeks |

**Deliverable**: 4 active strategies with automated rebalancing and rollover. Portfolio analytics reflect multi-strategy composition.

### Phase 3: Affiliate Sub-Vault MVP (Month 3–5)

See companion document: [`docs/affiliate-sub-vault-model.md`](./affiliate-sub-vault-model.md)

### Phase 4: Scale & Governance (Month 5+)

| Task | Description |
|------|-------------|
| Agent rate bidding | Market-based interest rate discovery for Affiliate loans |
| On-chain reputation | Agent credit scores based on repayment history |
| Cross-chain strategies (CCIP) | Deploy strategies on L2s for lower gas and additional yield sources |
| Governance integration | Community voting on strategy weights and agent approval |
| Dynamic allocation | AI-driven rebalancing based on market conditions (via Chainlink Functions) |
